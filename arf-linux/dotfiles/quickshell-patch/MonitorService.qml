...
Singleton {
  ...
  function buildLuaContent(edits) {
    ...
    for (const m of edits) {
      ...
      lines.push(`hl.exec_cmd("eval ${entry}")`);
    }
    return lines.join("\n") + "\n";
  }
  ...
  function apply(edits) {
    ...
    _lastEdits = edits;
    // Apply each monitor via hyprctl eval (works with Lua configs)
    const evalCmds = edits.map(m => `hyprctl eval '${buildMonitorArg(m)}'`).join(" && ");
