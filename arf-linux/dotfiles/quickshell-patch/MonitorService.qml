...
Singleton {
  ...
  function buildLuaContent(edits) {
    ...
    for (const m of edits) {
      const entry = buildMonitorArg(m);
      lines.push(`hl.exec_cmd("${entry}")`);
    }
    return lines.join("\n") + "\n";
  }
  ...
  function apply(edits) {
    ...
    _lastEdits = edits;
    // Apply each monitor via hyprctl eval + hl.exec_cmd (required for Lua configs)
    const evalCmds = edits.map(m =>
      `hyprctl eval 'hl.exec_cmd("${buildMonitorArg(m)}")'`
    ).join(" && ");
    applyProc.command = ["sh", "-c", evalCmds];
    applyProc.running = true;
    // Persist to monitors.lua so it survives reboots
    persistToFile(edits);
  }
